...
if ( is_array( $templates_array ) ) { ?>
	<div class="sktb-template-dir wrap">
        <h2 class="wp-heading-inline sktb-first-heading"> <?php echo apply_filters( 'sktb_template_dir_page_title', __( 'SKT Templates Directory', 'skt-templates' ) ); ?></h2>
            ...
            <form method="post" id="sktb-searchform-template"> 
                    	<span> 
                            ...
                            <input type="text" name="search" placeholder="Find Templates" value="" required>
        ...
        <div class="sktb-search-result">
        	<?php echo __( 'search result for <span>'.$search.'</span>', 'skt-templates' ); ?>
        	<a href="<?php echo esc_url($get_admin_url)."admin.php?page=skt_template_directory"?>"><img src="<?php echo esc_url( SKTB_URL ); ?>images/delete-search.png" title="" class="sktb-delete-search"/></a>
        </div>
    	<?php } ?>
			...
			foreach ( $templates_array as $template => $properties ) {
				...
				<div class="install-theme-info sktb-theme-info <?php echo esc_attr( $template ); ?>"
					...
					<div class="theme-details">
                    	<a href="<?php echo esc_url( $properties['theme_url'] ); ?>" target="_blank"><?php esc_html_e('Download free theme', 'skt-templates');?></a>
						<?php echo esc_html($properties['description']); ?>
                        <p><i><?php esc_html_e( 'Please wait for 10-15 seconds for the preview to load completely as it depends on your internet and hosting speed.', 'skt-templates' ); ?></i></p>
					</div>
					<?php
