...
final class JupiterX_Customizer_Utils {
	...
	public static function get_layouts( $choices = [] ) {
		$right   = is_rtl() ? esc_html__( 'Left', 'jupiterx-lite' ) : esc_html__( 'Right', 'jupiterx-lite' );
		$left    = is_rtl() ? esc_html__( 'Right', 'jupiterx-lite' ) : esc_html__( 'Left', 'jupiterx-lite' );
		$choices = $choices;

		$choices = array_merge( $choices, [
			'c'       => esc_html__( 'No sidebar', 'jupiterx-lite' ),
			/* translators: The sidebar position */
			'sp_c'    => sprintf( esc_html__( 'Single Sidebar %s', 'jupiterx-lite' ), $left ),
			/* translators: The sidebar position */
			'c_sp'    => sprintf( esc_html__( 'Single Sidebar %s', 'jupiterx-lite' ), $right ),
			/* translators: The sidebar position */
			'sp_ss_c' => sprintf( esc_html__( 'Double Sidebar %s', 'jupiterx-lite' ), $left ),
			/* translators: The sidebar position */
			'c_sp_ss' => sprintf( esc_html__( 'Double Sidebar %s', 'jupiterx-lite' ), $right ),
			'sp_c_ss' => esc_html__( 'Opposing Sidebars', 'jupiterx-lite' ),
		] );
	...
	public static function get_terms( $taxonomy, $add_default = true ) {
		...
		if ( $add_default ) {
			$terms = [ '0' => esc_html__( 'All Categories', 'jupiterx-lite' ) ] + $terms;
		}
