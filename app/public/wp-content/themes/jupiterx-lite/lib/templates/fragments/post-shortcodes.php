...
function jupiterx_post_meta_date_shortcode() {
	jupiterx_open_markup_e( 'jupiterx_post_meta_date_prefix', 'span' );
		...
		jupiterx_output_e( 'jupiterx_post_meta_date_prefix_text', esc_html__( 'Posted on ', 'jupiterx-lite' ) );
...
function jupiterx_post_meta_author_shortcode() {
	jupiterx_open_markup_e( 'jupiterx_post_meta_author_prefix', 'span' );
		...
		jupiterx_output_e( 'jupiterx_post_meta_author_prefix_text', esc_html__( 'By ', 'jupiterx-lite' ) );
...
function jupiterx_post_meta_comments_shortcode() {
	...
	if ( $comments_number < 1 ) {
		$comment_text = jupiterx_output( 'jupiterx_post_meta_empty_comment_text', esc_html__( 'Leave a comment', 'jupiterx-lite' ) );
	} elseif ( 1 === $comments_number ) {
		$comment_text = jupiterx_output( 'jupiterx_post_meta_comments_text_singular', esc_html__( '1 comment', 'jupiterx-lite' ) );
	} else {
		$comment_text = jupiterx_output(
			'jupiterx_post_meta_comments_text_plural',
			// translators: Number of comments. Plural.
			esc_html__( '%s comments', 'jupiterx-lite' )
		);
	}
...
function jupiterx_post_meta_tags_shortcode() {
	...
	printf(
		'%1$s%2$s',
		jupiterx_output( 'jupiterx_post_meta_tags_prefix', esc_html__( ' ', 'jupiterx-lite' ) ), // @codingStandardsIgnoreLine
...
function jupiterx_post_meta_categories_shortcode() {
	...
	$prefix .= jupiterx_output( 'jupiterx_post_meta_categories_prefix_text', esc_html__( 'In ', 'jupiterx-lite' ) ); // @codingStandardsIgnoreLine
...
function jupiterx_post_social_share_shortcode( $markup_key, $filtered_social = [], $name_enabled = true ) {
	...
	$social_networks = [
		'facebook'    => [
			'label'  => esc_attr__( 'Share on Facebook', 'jupiterx-lite' ),
			'name'   => esc_html__( 'Facebook', 'jupiterx-lite' ),
			'url'    => 'https://facebook.com/sharer/sharer.php?u=%1$s',
			'icon'   => 'facebook-f',
		],
		'twitter'     => [
			'label' => esc_attr__( 'Share on Twitter', 'jupiterx-lite' ),
			'name'  => esc_html__( 'Twitter', 'jupiterx-lite' ),
			'url'   => 'https://twitter.com/intent/tweet/?text=%2$s&url=%1$s',
			'icon'  => 'twitter',
		],
		'pinterest'   => [
			'label' => esc_attr__( 'Share on Pinterest', 'jupiterx-lite' ),
			'name'  => esc_html__( 'Pinterest', 'jupiterx-lite' ),
			'url'   => 'https://pinterest.com/pin/create/button/?url=%1$s&media=%1$s&description=%2$s',
			'icon'  => 'pinterest-p',
		],
		'linkedin'    => [
			'label' => esc_attr__( 'Share on LinkedIn', 'jupiterx-lite' ),
			'name'  => esc_html__( 'LinkedIn', 'jupiterx-lite' ),
			'url'   => 'https://www.linkedin.com/shareArticle?mini=true&url=%1$s&title=%2$s&summary=%2$s&source=%1$s',
			'icon'  => 'linkedin-in',
		],
		'reddit'      => [
			'label' => esc_attr__( 'Share on Reddit', 'jupiterx-lite' ),
			'name'  => esc_html__( 'Reddit', 'jupiterx-lite' ),
			'url'   => 'https://reddit.com/submit/?url=%1$s',
			'icon'  => 'reddit-alien',
		],
		'digg'        => [
			'label' => esc_attr__( 'Share on Digg', 'jupiterx-lite' ),
			'name'  => esc_html__( 'Digg', 'jupiterx-lite' ),
			'url'   => 'https://digg.com/submit?url=%1$s',
			'icon'  => 'digg',
		],
		'email'       => [
			'label'  => esc_attr__( 'Share on Email', 'jupiterx-lite' ),
			'name'   => esc_html__( 'Email', 'jupiterx-lite' ),
