...
function jupiterx_post_404_title() {
	...
	jupiterx_open_markup_e( 'jupiterx_404_title', 'h1', [ 'class' => 'jupiterx-404-title' ] );
		...
		jupiterx_output_e( 'jupiterx_404_title_text', esc_html__( '404', 'jupiterx-lite' ) ); // phpcs:ignore WordPress.XSS.EscapeOutput.OutputNotEscaped -- Pending security audit.
...
function jupiterx_post_archive_title() {
	...
	jupiterx_open_markup_e( 'jupiterx_archive_title', 'h1', [ 'class' => 'jupiterx-archive-title' ] );
...
function jupiterx_post_more_link( $output ) {
	...
	$output = jupiterx_open_markup(
		...
		$output .= jupiterx_output( 'jupiterx_post_more_link_text', esc_html__( 'Continue reading', 'jupiterx-lite' ) );
...
function jupiterx_post_content_navigation() {
	echo wp_link_pages(
		array(
			'before' => jupiterx_open_markup( 'jupiterx_post_content_navigation', 'p', array( 'class' => 'font-weight-bold' ) ) . jupiterx_output( 'jupiterx_post_content_navigation_text', esc_html__( 'Pages:', 'jupiterx-lite' ) ),
	...
	$output = jupiterx_open_markup(
		...
		$output .= jupiterx_open_markup( 'jupiterx_previous_body[_post_navigation]', 'div', 'class=jupiterx-post-navigation-body' );
			...
			$output .= jupiterx_open_markup( 'jupiterx_previous_label[_post_navigation]', 'span', 'class=jupiterx-post-navigation-label' );
				...
				$output .= jupiterx_output( 'jupiterx_previous_label_text[_post_navigation]', esc_html__( 'Previous', 'jupiterx-lite' ) );
	...
	$output = jupiterx_open_markup(
		...
		$output .= jupiterx_open_markup( 'jupiterx_next_body[_post_navigation]', 'div', 'class=jupiterx-post-navigation-body' );
			...
			$output .= jupiterx_open_markup( 'jupiterx_next_label[_post_navigation]', 'span', 'class=jupiterx-post-navigation-label' );
				...
				$output .= jupiterx_output( 'jupiterx_next_label_text[_post_navigation]', esc_html__( 'Next', 'jupiterx-lite' ) );
	...
	if ( get_previous_posts_link() ) {
		jupiterx_open_markup_e( 'jupiterx_posts_pagination_item[_previous]', 'li', [ 'class' => 'page-item' ] );
			...
			jupiterx_open_markup_e(
				...
				jupiterx_output_e( 'jupiterx_previous_text[_posts_pagination]', esc_html__( 'Previous', 'jupiterx-lite' ) );
	...
	if ( get_next_posts_link() ) {
		jupiterx_open_markup_e( 'jupiterx_posts_pagination_item[_next]', 'li', [ 'class' => 'page-item' ] );
			...
			jupiterx_open_markup_e(
				...
				jupiterx_output_e( 'jupiterx_next_text[_posts_pagination]', esc_html__( 'Next', 'jupiterx-lite' ) );
	...
	jupiterx_open_markup_e( 'jupiterx_post', 'article', [ 'class' => 'jupiterx-no-article jupiterx-post' ] );
		...
		jupiterx_open_markup_e( 'jupiterx_post_header', 'header' );
			...
			jupiterx_open_markup_e( 'jupiterx_post_title', 'h1', array( 'class' => 'jupiterx-post-title' ) );
				...
				jupiterx_output_e( 'jupiterx_no_post_article_title_text', esc_html__( 'Whoops, no result found!', 'jupiterx-lite' ) );
		...
		jupiterx_open_markup_e( 'jupiterx_post_body', 'div' );
			...
			jupiterx_open_markup_e( 'jupiterx_post_content', 'div', array( 'class' => 'jupiterx-post-content' ) );
				...
				jupiterx_open_markup_e( 'jupiterx_no_post_article_content', 'p' );
					...
					jupiterx_output_e( 'jupiterx_no_post_article_content_text', esc_html__( 'It looks like nothing was found at this location. Try a new search?', 'jupiterx-lite' ) );
...
function jupiterx_post_password_form() {
	...
	$output = jupiterx_open_markup( 'jupiterx_password_form_notice', 'p', array( 'class' => 'alert alert-warning' ) );
		...
		$output .= jupiterx_output( 'jupiterx_password_form_notice_text', esc_html__( 'This post is protected. To view it, enter the password below!', 'jupiterx-lite' ) );
	...
	$output .= jupiterx_open_markup(
		...
		$output .= jupiterx_selfclose_markup(
			...
			array(
				'class'       => 'form-control',
				'type'        => 'password',
				'placeholder' => esc_attr( apply_filters( 'jupiterx_password_form_input_placeholder', esc_html__( 'Password', 'jupiterx-lite' ) ) ), // Automatically escaped.
				'name'        => 'post_password',
			)
		);
		...
		$output .= jupiterx_selfclose_markup(
			...
			array(
				...
				'type'  => 'submit',
				'name'  => 'submit',
				'value' => esc_attr( apply_filters( 'jupiterx_password_form_submit_text', esc_html__( 'Submit', 'jupiterx-lite' ) ) ),
			)
		);
...
// Filter.
jupiterx_add_smart_action( 'post_gallery', 'jupiterx_post_gallery', 10, 3 );
/**
 ...
 * @SuppressWarnings(PHPMD.ElseExpression)
 */
function jupiterx_post_gallery( $output, $attr, $instance ) {
	...
	$output = apply_filters( 'gallery_style', $gallery_div ); // phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedHooknameFound -- Used in function scope.
		...
		$i = 0; foreach ( $attachments as $attachment_id => $attachment ) {
			...
			$attr        = ( trim( $attachment->post_excerpt ) ) ? [ 'aria-describedby' => "gallery-{$instance}-{$id}" ] : [];
